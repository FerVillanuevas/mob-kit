...
export default function CustomerAddresses() {
  ...
  if (isLoading) {
    return (
      <View className="flex flex-1 p-4">
        <Text className="mb-4 text-2xl font-bold">My Addresses</Text>
  ...
  if (!data || !data.addresses || data.addresses.length === 0) {
    return <EmptyHero title="No addresses" hideButton />;
  }
    ...
    <View className="flex flex-1">
      ...
      <FlashList
        ...
        renderItem={({ item }) => {
          return (
            <TouchableOpacity
              onPress={() => router.push(`/customer/address/${item.addressId}`)}
            ...
            >
              <View className="flex flex-row items-center justify-between">
                <View className="flex flex-col">
                  ...
                  <Text className="text-sm text-muted-foreground">
                    {item.address1}, {item.city}, {item.stateCode} {item.postalCode}
                  </Text>
                  <Text className="text-sm text-muted-foreground">
                    {item.countryCode}
                  </Text>
